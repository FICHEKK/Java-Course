...
public class GlasanjeGrafikaServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		PieDataset dataset = createDataset((List<PollOption>) req.getSession().getAttribute("pollOptionsSorted"));
     ...
     */
    private PieDataset createDataset(List<PollOption> pollOptions) {
        DefaultPieDataset result = new DefaultPieDataset();
        
        for(PollOption option : pollOptions) {
        	if(option.getVotes() > 0) {
        		result.setValue(option.title, option.getVotes());
        	}
        }
